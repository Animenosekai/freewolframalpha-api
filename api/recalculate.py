...
from constants import makeResponse, RECALCULATION_URL
from requests import get
from urllib.parse import quote

## Variable Initialization
LOCAL_CACHE = {} # store variable
CACHE_DURATION = 900 # in sec.
...
def hello(path):
    try:
        ...
        if "id" not in request.values:
            return makeResponse({"message": "'id' is required"}, error="MISSING_ARGUMENT", code=400)

        if "domain" not in request.values:
            return makeResponse({"message": "'domain' is required"}, error="MISSING_ARGUMENT", code=400)
        
        id = request.values.get("id")
        domain = request.values.get("domain")

        response = get(RECALCULATION_URL.format(host=domain, id=quote(id, safe='')))
